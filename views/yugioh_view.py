    ...
    with st.container():
        ...
        with col2:
            ...
            if card.has_stats:
                c1, c2 = st.columns(2)
                with c1:
                    st.markdown('<p class="stat-label">Ataque / ATK</p>', unsafe_allow_html=True)
                    st.markdown(f'<p class="stat-value">{card.atk if card.atk is not None else "0"}</p>', unsafe_allow_html=True)
                with c2:
                    st.markdown('<p class="stat-label">Defensa / DEF</p>', unsafe_allow_html=True)
                    st.markdown(f'<p class="stat-value">{card.defs if card.defs is not None else "0"}</p>', unsafe_allow_html=True)
                st.markdown("---")
            
            st.markdown("### ESPECIFICACIONES")
            st.markdown(f'<div class="desc-text">{card.desc}</div>', unsafe_allow_html=True)
            
            if card.cardmarket_price:
                st.markdown("<div style='margin-top: 1.5rem;'></div>", unsafe_allow_html=True)
                st.markdown(f"VALOR DE MERCADO: **${card.cardmarket_price} USD**")
...
def render_sidebar():
    ...
    with st.sidebar:
        st.image("https://images.ygoprodeck.com/images/assets/ygoprodeck_header_logo.png", width=200)
        ...
        selected = st.selectbox("CARTAS RELEVANTES", popular_cards, key="popular_select")
        if selected != "SELECCIONAR...":
            if st.button("VISUALIZAR DATOS", width='stretch'):
                navigate_to_card(selected)

def render_history_panel():
    """Panel de historial (Sin emojis)"""
    st.header("REGISTROS")
...
def render_search_section():
    """Buscador (Sin emojis)"""
    c_input, c_btns = st.columns([2.5, 1], gap="small", vertical_alignment="center")
    ...
    with c_btns:
        ...
        with b1:
            btn_random = st.button("AZAR", width='stretch', help="Ficha Aleatoria")
        with b2:
            btn_clear = st.button("RESET", width='stretch', help="Resetear Interfaz")
