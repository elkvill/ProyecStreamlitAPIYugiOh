...
def render_deck_section():
    ...
    for i, card in enumerate(st.session_state['mazo']):
        with st.container():
            ...
            copias = sum(
                1 for c in st.session_state['mazo'] if c.id == card.id)
            ...
            with col_info:
                ...
                st.markdown(
                    f"<small>{card.type}{stats_str}</small>", unsafe_allow_html=True)
            ...
            st.markdown(
                '<hr style="margin: 0.5rem 0; border: none; border-top: 1px solid #1e293b;">', unsafe_allow_html=True)


def render_deck_summary():
    """Muestra un resumen de la composición del mazo."""
